...
namespace qpt {
	...
	MCherryTree<Label>::~MCherryTree() {
		...
		consistent = false;
		/* Clear all blossom candidates */
		//while (!blossomCandidates.empty()) {
		//	blossomCandidates.pop();
		//}
	...
	* the children of the new nodes. */
	...
	void MCherryTree<Label>::adoptBlossom(MCherryBlossom<Label> * blossom) {
		...
		bool consistencyUpdate = this->consistent;
		this->consistent = false;
		std::function<void(MVertex<Label>*)> fun = [this, blossom](MVertex<Label> * v)->void {
			DEBUG("Visiting " << v->id << "\n");
			v->setContainingTree(this);
			v->enqueue();
			this->size += 1;
			/* Adopt further blossoms */
			for (ListElement<MCherryBlossom<Label>>* el = v->bearingBlossoms.getFirstElement(); el != nullptr; el = el->nxtElem) {
				this->adoptBlossom(el->value->getReference());
			}
			/* Add children to the tree */
			for (ListElement<HalfEdge<Label>>* elem = v->oddChildren.getFirstElement(); elem != nullptr; elem = elem->nxtElem) {
	...
	/* Checks wether or not a new edge could induce a new blossom */
	template <class Label>
	bool MCherryTree<Label>::inducesNewBlossom(HalfEdge<Label> * e) {
		MVertex<Label>* u = e->start;
		MVertex<Label>* w = e->end;
		if (u->getContainingTree() != this || w->getContainingTree() != this) {
			return false;
		}
	...
	/* Stores a blossom candidate for later use */
	template <class Label>
	void MCherryTree<Label>::rememberBlossom(HalfEdge<Label> * e) {
		if (inducesNewBlossom(e)) {
			blossomCandidates.push(e);
		}
	}

	/* Creates a blossom from the list of blossom candidates if possible
	 * according to the pseudocode. */
	template <class Label>
	void MCherryTree<Label>::makeBlossom() {
		if (!canCreateBlossom()) {
			return;
		}
		HalfEdge<Label>* e = blossomCandidates.top().e;
		blossomCandidates.pop();
		MVertex<Label>* u = e->start;
		MVertex<Label>* w = e->end;
		MCherryBlossom<Label>* uBlossom = u->getContainingBlossom();
	...
	/* Searches the blossom candidates for a blossom inducing edge. If found, returns true. Removes all unneccessary candidates */
	template <class Label>
	bool MCherryTree<Label>::canCreateBlossom() {
		while (!blossomCandidates.empty()) {
			if (inducesNewBlossom(blossomCandidates.top().e)) {
				return true;
			} else {
				blossomCandidates.pop();
			}
	...
	template <class Label>
	int MCherryTree<Label>::printCtr = 0;
